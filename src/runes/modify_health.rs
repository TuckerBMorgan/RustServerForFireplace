...
impl Rune for ModifyHealth {
    fn execute_rune(&self, game_state: &mut GameState) {
        if !game_state.is_ai_copy_running() && !game_state.get_minion(self.target_uid).unwrap().has_tag("HERO".to_string()){
            {
    ...
    fn to_bson_doc(&self, game_name: String, count: usize) -> Document{
        return to_doc(bson::to_bson(&self).unwrap(), game_name, count, "ModifyHealth".to_string());
    }
}
